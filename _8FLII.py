...
def P8(root, threshold):
    print('process8')
    # read_root = '/data/xtw/Project/pythonProject/0000/p6coor_result/MAP'
    # paths = os.listdir(read_root)
    # paths.sort()

    read_root = os.path.join(root, 'p7coor_result/coor/CD21.csv')
    if not os.path.exists(read_root):
        save_root = os.path.join(root, 'p8coor_result')
        ...
        return 0
    df0 = pd.read_csv(read_root, index_col=0)
    paths = df0['name'].values + '.npy'

    maskroot = os.path.join(root, 'p6_2corr_result_smooth/MAP')  # p6_corr
    CDroot = os.path.join(root, 'p3channel/CD23')
    ...
    for i in range(len(paths)):
        ...
        mask = np.load(os.path.join(maskroot, pathi))
